...
// Your file goes here.
$file = "C:\\gretil1\\dhatumeaning.txt";
...
{
$s[$i] = explode("#",$b[$i]);
$previousnumber[$i] = ltrim(chop($s[$i][0]));
$pada[$i]= ltrim(chop($s[$i][2]));
$meaning[$i] = ltrim(chop($s[$i][3]));
$url[$i] = ltrim(chop($s[$i][4]));
$dhatu[$i] = ltrim(chop($s[$i][1]));
$p[$i] = ltrim(chop($s[$i][1]));
$c[$i] = convert($p[$i]);
$c[$i] = json_encode($c[$i]);
...
// creating a multidimentional array $araay which contains $c,$previousnumber, $pada, $meaning, $url, $dhatu

$i=0;
while ($i<count($b))
{
$array[$i] = array('$c' => $c[$i], '$previousnumber' => $previousnumber[$i], '$pada' => $pada[$i], '$meaning' => $meaning[$i], '$url' => $url[$i], '$dhatu' => $dhatu[$i]);
$i++;
}
...
{
$output[$i] = $array[$i]['$previousnumber']." # ".$array[$i]['$dhatu']." # ".$array[$i]['$pada']." # ".$array[$i]['$meaning']." # ".$array[$i]['$url'];
}
$out = implode("\r\n",$output);
file_put_contents("C:\\devanagarisorted.txt",$out);
?>
