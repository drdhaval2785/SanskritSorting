...
$a = file_get_contents("C:\\oliv.html");
$b = explode("<body>",$a);
//echo $b[0]."<br>".$b[1];
$qq = explode("<hr></p>",$b[1]);
//print_r($qq);
foreach ($qq as $val1)
{  
$d = explode('<span class="link">',$val1);
//print_r($d);
$e = $d[0];
//echo $e;
array_shift($d);
...
foreach($d as $value)
{
$f[] = explode('</span>  - <span class="quote">',$value);
}
...
while ($i<count($d))
{
$array[$i] = array('$c' => $c[$i], '$ref' => $ref[$i], '$meaning' => $post[$i]);
$i++;
} unset($c); unset($ref); unset($post);
//print_r($array);
...
{
    $ddd[$i+1] = $array[$i]['$ref'].'</span>  - <span class="quote">'.$array[$i]['$meaning'];
}
unset($array);
//print_r($ddd);
...
$aa = $b[0].$bb;
//echo $aa;
file_put_contents("C:\\oliver1.html",$aa);
?>