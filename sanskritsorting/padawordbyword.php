...
$test = file("C:\\gretil1\\devanagaridhatulist.txt");
...
{ // reads the 'i'th member of the array. $test[0] would mean the first member of the array.
   $text = $test[$i];
                 ...
                 */
$numerals = '0123456789';
$alphabets = 'AĀIĪUŪRṚEOKGṄCJÑṬḌṆTDNPBMYLVŚṢSHaāiīuūṛeoṁḥkgṅcjñṭḍṇtdnpbmyrlvśṣah';
// If you want the abbreviations to start from numbers - run this part of code and comment the next.
$z = preg_split('/(['.$numerals.']([^\r\n]*)[\r\n])/u', $text, -1,PREG_SPLIT_DELIM_CAPTURE );
// If you want the abbreviations to start from the alphabets, use this and comment the preceding one.
//$z = preg_split('/(['.$alphabets.']([^\r\n]*)[\r\n])/u', $text, -1,PREG_SPLIT_DELIM_CAPTURE );
//print_r($z);
$original[$i] = $z[0];
if (count($z)>1)
{$post[$i] = $z[1];}
